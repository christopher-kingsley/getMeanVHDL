...
entity myCounterModule is
    port(
        ...
        address     : out std_logic_vector(5 downto 0); -- Current RAM address
        count       : out std_logic_vector( downto 0); -- Count of nonzero values
        counter_done: out std_logic -- Signals when all addresses have been checked
    );
end entity;

architecture beh of myCounterModule is
    signal addr_reg : unsigned(5 downto 0) := (others => '0'); -- Address tracker (3 bits)
    signal count_reg : unsigned(5 downto 0) := (others => '0'); -- Nonzero counter
begin
    process(clk)
    begin
        if rising_edge(clk) then
            if reset = '1' then
                ...
                if addr_reg < "111" then -- Max address is 7 (3 bits)
                    ...
                    addr_reg <= "0000000000000000"; -- Reset address to 0 if needed
                    counter_done <= '1';
                end if;
            end if;
        end if;
